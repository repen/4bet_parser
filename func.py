...
def marathon(html):
    ...
    if result:
        koef = result.parent.parent.parent.parent.parent.select_one('span[data-selection-key*="Total_Sets.Over_2.5"]')
        if koef:
...
def ligastavok(html):
    ...
    if result:
        res = re.findall(r"\d\,\d{1,4}", str(result.parent.select_one('div[title*="бол"]')))
        if res:
            value = res[0]
...
def xbet(html):
    ...
    if result:
        ...
        res = re.findall(r"\d\.\d{1,4}", str(result.parent.parent.find(text=re.compile('^3\n')).parent.parent ) )
        if res:
            value = res[0]
